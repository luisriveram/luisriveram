...
$("#ardoraQuest").html("<img id='playSound' class='imaLeft' src='Scratch_resources/media/' alt='Sound' align='left'>"+ "<p>" + questionW[indexG] + "</p>");
...
ind++;if (ind==iW.length){ind=0;}}wordsGL[ind]=iWL[i];wordsG[ind]=iW[i];imageW[ind]=imaW[i];questionW[ind]=queW[i];audioW[ind]=auW[i];}
}
function calcMaxWidth(){for (i = 0; i < wordsG.length; i++) {var actualSize=0;actualSize=$.trim(aforWords(wordsG[i])).length*(52+9);
...
function paintbGame(){profG=profG+40+9;var leV=(actMaxWidth-($.trim(aforWords(wordsG[indexG])).length-1)*(40+19))/2;
